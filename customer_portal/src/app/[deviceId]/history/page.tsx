...
// Max allowed range in milliseconds (12 hours)
const MAX_RANGE_MS = 12 * 60 * 60 * 1000;

// Helper: get today's start (00:00) as datetime-local string
...
// Helper: get current time as datetime-local string
function getNowString(): string {
  return toLocalDatetimeString(new Date());
}

// Helper: convert Date to datetime-local input string (YYYY-MM-DDTHH:MM)
...
export default function HistoryPage() {
  ...
  // Historical data state (only loaded on user action)
  ...
  // Date range state — defaults to today (00:00 → now), max 12 hours
  const [fromDate, setFromDate] = useState<string>(getTodayStart);
  const [toDate, setToDate] = useState<string>(getNowString);
  ...
  // Validate date range (max 12 hours)
  const validateDateRange = useCallback((from: string, to: string): string | null => {
    const fromMs = new Date(from).getTime();
    const toMs = new Date(to).getTime();

    if (isNaN(fromMs) || isNaN(toMs)) return 'Invalid date selection.';
    if (fromMs >= toMs) return '"From" must be before "To".';
    if (toMs - fromMs > MAX_RANGE_MS) return 'Maximum allowed range is 12 hours. Please narrow your selection.';

    return null;
  }, []);

  // Handle "From" date change with auto-clamping
  const handleFromDateChange = (value: string) => {
    setFromDate(value);
    const error = validateDateRange(value, toDate);
    setDateRangeError(error);
  };

  // Handle "To" date change with auto-clamping
  const handleToDateChange = (value: string) => {
    setToDate(value);
    const error = validateDateRange(fromDate, value);
    setDateRangeError(error);
  };
  ...
  // Load historical data — ONLY called when user clicks "Load Data"
  ...
  }, [deviceId, fromDate, toDate, validateDateRange]);
  ...
  // Initial load — only device status & predictions, NOT historical data
  useEffect(() => {
    if (isAuthenticated) {
      const init = async () => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-8 space-y-6">
        ...
        {/* ── Section 1: Predictions & Analysis ── */}
        <div className="card">
          ...
          {predictions ? (
            <div className="space-y-5">
              {/* Current Status — horizontal cards */}
              {latestTelemetry && (
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="bg-gray-50 rounded-lg p-3">
                      <span className="text-gray-500 text-xs">Room Temp</span>
                      <p className="font-semibold text-lg">{latestTelemetry.roomTemp?.toFixed(1) || 'N/A'}°C</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3">
                      <span className="text-gray-500 text-xs">Power</span>
                      <p className="font-semibold text-lg">{latestTelemetry.powerWatts?.toFixed(0) || 'N/A'} W</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3">
                      <span className="text-gray-500 text-xs">Humidity</span>
                      <p className="font-semibold text-lg">{latestTelemetry.humidity?.toFixed(0) || 'N/A'}%</p>
                    </div>
                    <div className="bg-gray-50 rounded-lg p-3">
                      <span className="text-gray-500 text-xs">Energy</span>
                      <p className="font-semibold text-lg">{latestTelemetry.energyKwh?.toFixed(2) || 'N/A'} kWh</p>
                    </div>
                  </div>
                </div>
              )}

              {/* Metrics row — horizontal layout for desktop */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <div className="bg-blue-50 rounded-lg p-4 h-[calc(100%-28px)] flex flex-col justify-center">
                ...
                <div>
                  ...
                  <div className="bg-yellow-50 rounded-lg p-4 space-y-2 h-[calc(100%-28px)]">
        ...
        {/* ── Section 2: Date Range Selection (max 12 hours) ── */}
        <div className="card">
          <h2 className="text-lg font-semibold mb-4 flex items-center">
            <i className="lni lni-calendar mr-2 text-primary"></i>
            Historical Data Range
            <span className="ml-2 text-sm font-normal text-gray-500">(max 12 hours)</span>
          </h2>
          <div className="flex flex-wrap gap-4 items-end">
            ...
            <button
              ...
              disabled={historyLoading || !!dateRangeError}
          ...
          {dateRangeError && (
            <div className="mt-3 flex items-center text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
        ...
        <div className="card">
          ...
          {selectedColumns.size === 0 ? (
            ...
            <div className="text-center py-12 text-gray-500">
              <i className="lni lni-calendar text-4xl mb-2"></i>
              <p>Select a date range and press <strong>Load Data</strong> to view historical records</p>
              <p className="text-sm mt-1">Maximum range: 12 hours</p>
            </div>
